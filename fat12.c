...
void longTraverse(unsigned int fileInfoDec[], char fileInfo[])
{
...
void structure(char* flag)
{
	...
	printf("\n	Number of Sectors per FAT: 	%d", newDecValue);
	...
	for (a = 19; a < 33; ++a) //reads through sectors 19-32 (root sectors)
	{
		...
		for (y = 0; y < 16; ++y) //16 file entries per sector
		{
			...
			if (flag == 1) //extended output flag will result in longTraverse function call
				longTraverse(fileInfoDec, fileInfo);
		}	
	}
	printf("\n");
}
...
void showfile(char arg[])
{
	unsigned int a,f,g,y,z = 0;  //variables used for loop counters
	unsigned char fileInfo[32] = {'\0'}; //char array to store a root entry at a time
	for (a = 19; a < 33; ++a) //reads through sectors 19-32 (root sectors)
	{
		lseek(3, 512*a, SEEK_SET); //changes start position to beginning of sector 'a'
		for (y = 0; y < 16; ++y) //16 file entries per sector
		{
			char filename[11] = {'\0'};
			lseek(3, (512*a + y*32), SEEK_SET); //start position is set to beginning of current sector plus y*32 bytes
			read(3, filename, 11); //read first 11 bytes of root entry into filename first
			filename[11] = '\0'; //null terminates the filename in case of garbage values in later indices
			lseek(3, (512*a + y*32), SEEK_SET); //reset position to start of root entry
			read(3, fileInfo, 32); //reads entire root entry into fileInfo
			if (fileInfo[0] == '\0') //null file entry
				continue;
			unsigned int location = 0; //will hold return value for starting sector location
			location = multiByteHexToDec(fileInfo, 27, 2, &location); //get sector location
			//The following if-statements and while loops format the users argument to match the format that is stored in filename
            if(arg[0] == '/')//block removes leading / character if present
            {
                for(g = 0; g < 13; ++g)
                arg[g] = arg[g+1];
            }
            char arg1[11]; //new character array to store formatted argument
            for (f = 0; f < 11; ++f) //initializes it to all space characters
                arg1[f] = ' ';
            f = 0;
            while (arg[f] != '.' && arg[f] != '\0') //read in file name portion of argument
            {
                arg1[f] = arg[f];
                ++f;
            }
            if (arg[f] == '.') //read in extension portion of argument
            {
                g = 8;
                ++f;
                while (arg[f] != '\0' && arg[f] != ' ')
            ...
            arg1[11] = '\0'; //null terminates the formatted filename
			if(strcasecmp(filename,arg1)==0)//file with the same name
			{
				unsigned int fileSize = 0;
				fileSize = multiByteHexToDec(fileInfo, 31, 4, &fileSize); //get fileSize from root entry
				for (f = 0; f <= (fileSize/512); ++f) //iterates a number of times equal to how many sectors the file takes up
				{
					char * buffer = malloc(512); //reserve space for buffer
					lseek(3, (31*512 + (f+location)*512), SEEK_SET); //move to appropriate start position
					read(3, buffer, 512); //reads 512 bytes (the entire sector) into the buffer
					for(g = 0; g < 512; ++g) //prints each character read in as hex values, rows of 16
					{
						if(g % 16 == 0)
							printf("\n     ");
						printf("%02X ", (unsigned char)buffer[g]); //casts to unsigned to avoid leading F's in certain characters
					}
					printf("\n");
				}
				lseek(3, 0, SEEK_SET); //resets to beginning of floppy
				printf("\n");
				return; //after file has been printed, no need for further root searches
			}
		}	
	}
printf("\nFile '%s' not found.\n", arg);
